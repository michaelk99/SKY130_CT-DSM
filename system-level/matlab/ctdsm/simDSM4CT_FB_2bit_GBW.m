...
function [y, fs, N, w1, w2, w3, w4] = simDSM4CT_FB_2bit_GBW(OSR, ctSteps, fb, fin, fres, amplitude, offset, omCoeff, aCoeff, b1, Vref, noiseVec_rms, wLimVec, cVec, optNoiseVec)
...
for a = 3:N
    for b = 1:nr_steps
        ...
        idx1 = ix-nr_steps*2;
        idx2 = ix-1;
        w4(ix) = w4(ix-1) + omega4*(    w3(idx1:idx2) - k4*DAC(idx1:idx2)   + noiseVec_rms(4)*randn()*sqrt(nr_steps) ) *integ_filter_4;
        w3(ix) = w3(ix-1) + omega3*(    w2(idx1:idx2) - k3*DAC(idx1:idx2)   + noiseVec_rms(3)*randn()*sqrt(nr_steps) ) *integ_filter_3;
        w2(ix) = w2(ix-1) + omega2*(    w1(idx1:idx2) - k2*DAC(idx1:idx2)   + noiseVec_rms(2)*randn()*sqrt(nr_steps) ) *integ_filter_2;
        w1(ix) = w1(ix-1) + omega1*( b1*in(idx1:idx2) - k1*DAC(idx1:idx2)   + noiseVec_rms(1)*randn()*sqrt(nr_steps) ) *integ_filter_1;
    ...
    % 2-bit mid-rise DAC
    [y(ix+1:ix+nr_steps), thermoCode] = mbq(w4(ix), 4, Vref);
    
    dacVal = Vref.*thermoDAC(thermoCode, elements, 4, false);
