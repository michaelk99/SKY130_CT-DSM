...
% Simulation
% ************************************************************
[y, fs, N, w1, w2, w3, w4, wsum, w1_in] = simDSM4CT_FF_2bit(OSR, ctSteps, fb, fin, fres, amplitude, offset, 0, 50, omCoeff, cCoeff, Vref, A0Vec, 0, noiseVec_rms, wLimVec, [false false false false], false);
...
cCoeffScaled = [0.6713 0.2495 0.1239 0.2582];   % 2bit Quant Vref = 0.36, 0.3 0.5 1 1 max swing
% omCoeffScaled = [s1 s2 s3 s4];                
omCoeffScaled = [5.89 5.89 2.64 0.313].*1e4;    % 2bit Quant Vref = 0.36, 0.3 0.5 1 1 max swing
wLimVec = 2.*intOutMaxVec;
%%
noiseEnVec = [true false false false];
noiseVec_rms(1) = 0.01e-6;
[y, fs, N, w1, w2, w3, w4, wsum, w1_in] = simDSM4CT_FF_2bit(OSR, ctSteps, fb, fin, fres, amplitude, offset, 0, 50, omCoeffScaled, cCoeffScaled, Vref, A0Vec, 0, noiseVec_rms, wLimVec, noiseEnVec, false);
plotInternalSignals(fs, fin, ctSteps, [w1_in; w1; w2; w3; w4], ["Input Int. 1", "Output Int. 1", "Output Int. 2", "Output Int. 3", "Output Int. 4"], true)
...
for i=1:nofruns
    ...
    disp(txt)
    [y, fs, N] = simDSM4CT_FF_2bit(OSR, ctSteps, fb, fin, fres, amplVec(i), offset, interferer_ampl, finter, omCoeffScaled, cCoeffScaled, Vref, A0Vec, eld, noiseVec_rms, wLimVec, [false false false false], false);
...
SQNR = SNRVec;
T1 = table(Input, SQNR);
writetable(T1, 'modMSAVerifyScaledFF.txt', 'Delimiter', ' ','WriteRowNames',false);
...
for i=1:length(finVec)
   [y, fs, N] = simDSM4CT_FF_2bit(OSR, ctSteps, fb, finVec(i), fres, amplitude, offset, interferer_ampl, finter, omCoeffScaled, cCoeffScaled, Vref, A0Vec, 0, noiseVec_rms, wLimVec, [false false false false], false);
...
for i=1:length(A0_Vec)
   [y, fs, N] = simDSM4CT_FF_2bit(OSR, ctSteps, fb, fin, fres, amplitude, offset, interferer_ampl, finter, omCoeffScaled, cCoeffScaled, Vref, [A0_Vec(i) A0_fixed A0_fixed A0_fixed], 0, noiseVec_rms, wLimVec, [false false false false], true);
   SNRVec1(i) = calcPSDParam(y, N, fs, fb, fin, fres, FS);
   clear y
   [y, fs, N] = simDSM4CT_FF_2bit(OSR, ctSteps, fb, fin, fres, amplitude, offset, interferer_ampl, finter, omCoeffScaled, cCoeffScaled, Vref, [A0_fixed A0_Vec(i) A0_fixed A0_fixed], 0, noiseVec_rms, wLimVec, [false false false false], true);
   SNRVec2(i) = calcPSDParam(y, N, fs, fb, fin, fres, FS);
   clear y
   [y, fs, N] = simDSM4CT_FF_2bit(OSR, ctSteps, fb, fin, fres, amplitude, offset, interferer_ampl, finter, omCoeffScaled, cCoeffScaled, Vref, [A0_fixed A0_fixed A0_Vec(i) A0_fixed], 0, noiseVec_rms, wLimVec, [false false false false], true);
   SNRVec3(i) = calcPSDParam(y, N, fs, fb, fin, fres, FS);
   clear y
   [y, fs, N] = simDSM4CT_FF_2bit(OSR, ctSteps, fb, fin, fres, amplitude, offset, interferer_ampl, finter, omCoeffScaled, cCoeffScaled, Vref, [A0_fixed A0_fixed A0_fixed A0_Vec(i)], 0, noiseVec_rms, wLimVec, [false false false false], true);
...
writetable(T1, 'modSNRvsA0_FF.txt', 'Delimiter', ' ','WriteRowNames',false);
...
for i=1:nofruns
   txt = sprintf("Run %d/%d", i, nofruns);
   disp(txt)
   [y, fs, N] = simDSM4CT_FF_2bit_GBW(OSR, ctSteps, fb, fin, fres, amplitude, offset, omCoeffScaled, cCoeffScaled, Vref, noiseVec_rms, wLimVec, [cVec(i) cFixed cFixed cFixed], noiseEnVec);
   SNRVec1(i) = calcPSDParam(y, N, fs, fb, fin, fres, FS);
   clear y
   [y, fs, N] = simDSM4CT_FF_2bit_GBW(OSR, ctSteps, fb, fin, fres, amplitude, offset, omCoeffScaled, cCoeffScaled, Vref, noiseVec_rms, wLimVec, [cFixed cVec(i) cFixed cFixed], noiseEnVec);
   SNRVec2(i) = calcPSDParam(y, N, fs, fb, fin, fres, FS);
   clear y
   [y, fs, N] = simDSM4CT_FF_2bit_GBW(OSR, ctSteps, fb, fin, fres, amplitude, offset, omCoeffScaled, cCoeffScaled, Vref, noiseVec_rms, wLimVec, [cFixed cFixed cVec(i) cFixed], noiseEnVec);
   SNRVec3(i) = calcPSDParam(y, N, fs, fb, fin, fres, FS);
   clear y
   [y, fs, N] = simDSM4CT_FF_2bit_GBW(OSR, ctSteps, fb, fin, fres, amplitude, offset, omCoeffScaled, cCoeffScaled, Vref, noiseVec_rms, wLimVec, [cFixed cFixed cFixed cVec(i)], noiseEnVec);
   SNRVec4(i) = calcPSDParam(y, N, fs, fb, fin, fres, FS);
   clear y
end
...
% noise2rmsVec = linspace(5e-6, 50e-6, 50);
...
for i=1:nofruns
    ...
    noiseVec_rms(2) = 30e-6;%noise2rmsVec(i);
    noiseVec_rms(3) = 200e-6;%noise3rmsVec(i);
    noiseVec_rms(4) = 300e-6;%noise4rmsVec(i);
    [y, fs, N] = simDSM4CT_FF_2bit(OSR, ctSteps, fb, fin, fres, amplitude, offset, interferer_ampl, finter, omCoeffScaled, cCoeffScaled, Vref, A0Vec, eld, noiseVec_rms, wLimVec, enableVec, true);
    SNRVec(i) = calcPSDParam(y, N, fs, fb, fin, fres, FS);
    disp(SNRVec(i))
    clear y
end
...
%% Export Data for latex doc
vn_rms = noise2rmsVec(1:40)';
SNR = SNRVec(1:40);
T1 = table(vn_rms, SNR);
writetable(T1, 'modSNRvsNoise2_FF.txt', 'Delimiter', ' ','WriteRowNames',false);
%% Export Data for latex doc
vn_rms = noise3rmsVec';
SNR = SNRVec;
T1 = table(vn_rms, SNR);
writetable(T1, 'modSNRvsNoise3_FF.txt', 'Delimiter', ' ','WriteRowNames',false);
%% Export Data for latex doc
vn_rms = noise4rmsVec';
SNR = SNRVec;
T1 = table(vn_rms, SNR);
